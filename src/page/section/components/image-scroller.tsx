...
function ImageScroller({
  ...
  const getClassStyle = useCallback(
    (className: string) => {
  ...
  return (
    <div className={getClassStyle("images-div-parent")}>
      {arrowNavigation ? (
        <div className={getClassStyle("arrow-navigation-div")}>
          <div className={roundedSquareTransitionClass}>
            <button
              className={[
                getClassStyle("arrow-img-button"),
                getClassStyle("left-button"),
              ].join(" ")}
              onClick={() => {
                scrollIndex(-1);
              }}
            >
              <img
                src={arrow}
                className={[
                  getClassStyle("arrow-img"),
                  theme === "dark" ? getClassStyle("inverted") : undefined,
                ].join(" ")}
                draggable="false"
              />
            </button>
      ...
      <div
        ref={setImagesDiv}
        tabIndex={-1}
        className={getClassStyle("images-div")}
        style={widthStyle}
      >
          ...
          <InteractiveImage
            ...
            customStyle={
              index == 0 && centerFirstImage && width
                ? {
                    marginLeft:
                      String((width - image.aspectRatio * height) / 2) + "px",
                  }
                : undefined
            }
            customClass={[
              index == 0
                ...
                : index == images.length - 1
                  ? styles["last-image"]
                  : undefined,
              getClassStyle("interactive-image"),
            ].join(" ")}
          />
        ))}
      </div>
