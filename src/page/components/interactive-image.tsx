				...
				<button onClick={() => { setSelectedImage(image); onImageClick(true); }}>
					<LazyLoad placeholder={<ImagePlaceholder image={image} />} scrollContainers={[scrollContainer, "#main-page"]} offset={300}>
						<div className={styles["image-loading-position"]}>
							...
							<img style={{ width: String(image.aspectRatio * image.height) + 'px' }} className={[
								styles["image"], !imageLoaded ? styles["hidden"] : undefined,
								image.dropShadowWithTheme && (image.dropShadowWithTheme === "both" || image.dropShadowWithTheme === theme) ? styles["drop-shadow"] : undefined,
							].join(' ')}
