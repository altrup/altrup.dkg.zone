...
const templateProduction = isProduction
	? await fs.readFile('./index.html', 'utf-8')
	: '';
const inlineCSSProduction = isProduction
	? await fs.readFile('./src/themes.css', 'utf-8')
	: '';
...
app.use('*', async (req, res) => {
	...
	try {
		let template, inlineCSS, inlineJS;
		if (!isProduction) {
			template = await fs.readFile('./index.html', 'utf-8');
			template = await vite.transformIndexHtml(url, template);

			inlineCSS = await fs.readFile('./src/themes.css', 'utf-8');
